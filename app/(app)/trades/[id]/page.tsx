...
const STATUS_LABEL: Record<string, string> = {
  pending: 'Pending',
  vendor_paid: 'Vendor paid — confirm receipt',
  confirmed: 'Confirmed',
  settled: 'Settled',
  disputed: 'Disputed',
  cancelled: 'Cancelled',
  ...
  return (
    <div style={{ paddingBottom: '5rem' }}>
      <div className="page-header">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
          <div>
            ...
            <div style={{ fontSize: '0.8125rem', color: 'rgba(255,255,255,0.5)' }}>Vendor: {trade.vendor?.name}</div>
          </div>
      ...
      <div style={{ padding: '1.25rem', display: 'flex', flexDirection: 'column', gap: '1rem' }}>
        ...
        {trade.status === 'vendor_paid' && (
          <div className="card" style={{ padding: '1.25rem', border: '1.5px solid var(--emerald)' }}>
            <div className="t-subhead" style={{ marginBottom: '0.375rem' }}>Vendor has marked as paid</div>
            <div className="t-caption" style={{ marginBottom: '1rem' }}>Confirm you received the payment to close this trade.</div>
            <button onClick={() => action('confirm_received')} disabled={actioning}
              ...
              {actioning ? 'Confirming…' : 'Confirm received'}
            </button>
          </div>
        )}

        {trade.status === 'pending' && trade.type === 'deposit' && (
          <div className="card" style={{ padding: '1.25rem', border: '1.5px solid var(--emerald)', marginBottom: '0' }}>
            <div className="t-subhead" style={{ marginBottom: '0.375rem' }}>Action required — make your transfer</div>
            <div className="t-caption" style={{ marginBottom: '1rem' }}>Transfer the exact amount below to the vendor's bank account. Your wallet will be credited once the vendor confirms receipt.</div>
            <div style={{ background: 'var(--emerald)', borderRadius: 'var(--r-md)', padding: '1.25rem', color: 'var(--white)', marginBottom: '0.875rem' }}>
              <div style={{ fontSize: '0.625rem', fontWeight: 700, color: 'rgba(255,255,255,0.5)', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: '0.25rem' }}>{(trade.vendor as {bank_name:string})?.bank_name}</div>
              <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.75rem', color: 'var(--white)', letterSpacing: '0.05em', marginBottom: '0.25rem' }}>{(trade.vendor as {account_number:string})?.account_number}</div>
              <div style={{ fontSize: '0.9375rem', fontWeight: 600, color: 'rgba(255,255,255,0.8)' }}>{(trade.vendor as {account_name:string})?.account_name}</div>
            </div>
            ...
            <button
              onClick={async () => {
                const fd = new FormData()
                fd.append('body', "I have made the transfer of " + formatNaira(trade.naira_amount) + ". Please confirm receipt.")
                await fetch(`/api/p2p/trade/${trade.id}/message`, { method: 'POST', body: fd })
                load()
              }}
              style={{ width: '100%', background: 'var(--emerald)', color: 'var(--white)', border: 'none', borderRadius: 'var(--r-md)', padding: '0.875rem', fontSize: '0.9375rem', fontWeight: 700, cursor: 'pointer' }}>
      ...
      {trade.status === 'settled' && (
          <div className="alert alert-success">This trade is settled and closed.</div>
        )}
        ...
        {trade.status === 'disputed' && (
          <div className="alert alert-error">Dispute open — admin will review and resolve.</div>
        )}
              ...
              <div key={m.id} style={{ display: 'flex', flexDirection: 'column', alignItems: m.sender_role === 'user' ? 'flex-end' : 'flex-start' }}>
                ...
                <div style={{ background: m.sender_role === 'user' ? 'var(--emerald)' : 'var(--sand)', color: m.sender_role === 'user' ? 'var(--white)' : 'var(--ink)', borderRadius: 'var(--r-sm)', padding: '0.625rem 0.875rem', maxWidth: '80%' }}>
                  ...
                  {m.attachment_url && m.attachment_type === 'image' && (
                    <img src={m.attachment_url} alt="attachment" style={{ maxWidth: '100%', borderRadius: 6, marginTop: m.body ? '0.5rem' : 0 }} />
                  )}
                  {m.attachment_url && m.attachment_type === 'document' && (
                    <a href={m.attachment_url} target="_blank" rel="noreferrer" style={{ display: 'block', fontSize: '0.8125rem', color: m.sender_role === 'user' ? 'rgba(255,255,255,0.8)' : 'var(--emerald)', marginTop: m.body ? '0.5rem' : 0 }}>
                      📄 View document
                    </a>
                  )}
                </div>
