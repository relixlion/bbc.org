  ...
  return (
    <div>
      <div style={{ marginBottom: '1.5rem' }}>
        <button onClick={() => router.back()} style={{ display: 'flex', alignItems: 'center', gap: '0.375rem', background: 'none', border: 'none', color: 'var(--ink-3)', fontSize: '0.8125rem', cursor: 'pointer', marginBottom: '0.75rem', padding: 0 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><polyline points="15,18 9,12 15,6"/></svg>
          Back
        </button>
        <div style={{ fontFamily: 'var(--font-display)', fontSize: '1.75rem', color: 'var(--ink)', letterSpacing: '-0.03em', marginBottom: '0.25rem' }}>
          {isDeposit ? 'Deposit' : 'Withdrawal'} · {formatNaira(trade.naira_amount)}
        </div>
        <div style={{ fontSize: '0.8125rem', color: 'var(--ink-3)' }}>Status: {trade.status.replace('_', ' ')}</div>
      </div>

      {isDeposit ? (
        <div style={{ background: 'var(--white)', border: '1.5px solid var(--emerald)', borderRadius: 'var(--r-lg)', padding: '1.25rem', marginBottom: '1rem' }}>
          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--emerald)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.875rem' }}>Send USDT "TRC20" to our wallet</div>
          ...
          <div style={{ background: 'var(--sand)', borderRadius: 'var(--r-sm)', padding: '0.75rem 1rem', wordBreak: 'break-all', fontSize: '0.875rem', fontFamily: 'monospace', color: 'var(--ink-2)' }}>
            {trade.vendor?.usdt_address ?? 'Wallet address not configured'}
          </div>
        </div>
      ) : (
        <div style={{ background: 'var(--white)', border: '1px solid var(--sand-2)', borderRadius: 'var(--r-lg)', padding: '1.25rem', marginBottom: '1rem' }}>
          <div style={{ fontSize: '0.75rem', fontWeight: 700, color: 'var(--ink-4)', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '0.875rem' }}>Pay to user</div>
      ...
      {trade.status === 'pending' && (
        <button onClick={markPaid} disabled={actioning}
          style={{ width: '100%', background: 'var(--emerald)', color: 'var(--white)', border: 'none', borderRadius: 'var(--r-md)', padding: '0.875rem', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer', marginBottom: '1rem' }}>
          {actioning ? 'Updating…' : isDeposit ? 'I have sent the USDT  "TRC20"' : 'I have paid the user'}
        </button>
      )}
      ...
      {trade.status === 'vendor_paid' && (
        <div style={{ background: 'var(--emerald-bg)', border: '1px solid #B7DFD0', borderRadius: 'var(--r-md)', padding: '0.875rem 1rem', marginBottom: '1rem', fontSize: '0.875rem', fontWeight: 600, color: 'var(--emerald-2)' }}>
          Marked as paid — waiting for user confirmation
        </div>
      )}
