  ...
  function SaveBtn({ k, value, label }: { k: string; value: unknown; label?: string }) {
    return (
      <button onClick={() => saveSetting(k, value)} disabled={saving === k}
        style={{ padding: '0.625rem 1.25rem', borderRadius: 'var(--r-sm)', background: saved === k ? 'var(--emerald-bg)' : 'var(--emerald)', color: saved === k ? 'var(--emerald-2)' : 'var(--white)', border: 'none', fontSize: '0.875rem', fontWeight: 700, cursor: 'pointer', transition: 'all 0.2s', opacity: saving === k ? 0.7 : 1 }}>
  ...
  return (
    <div>
      <div style={{ marginBottom: '1.75rem' }}>
        ...
        <div className="t-caption">All values are editable and take effect immediately</div>
      </div>
        ...
        <div className="card" style={{ padding: '1.25rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1rem' }}>
            <div>
              ...
              <div className="t-caption">Each amount is a fixed withdrawal unit. Users pick from available amounts on allowed days.</div>
            </div>
            ...
            {(s.withdrawal_thresholds ?? []).map((t, i) => (
              <div key={i} style={{ background: 'var(--sand)', borderRadius: 'var(--r-md)', padding: '1rem' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem' }}>
                  ...
                  <button onClick={() => { const arr = [...s.withdrawal_thresholds]; arr.splice(i, 1); setS({ ...s, withdrawal_thresholds: arr }) }}
                    style={{ fontSize: '0.75rem', fontWeight: 600, color: 'var(--danger)', background: 'none', border: 'none', cursor: 'pointer' }}>Remove</button>
                </div>
                <Input label="Amount (₦)" type="number" value={String(t.amount)}
                  onChange={e => { const arr = [...s.withdrawal_thresholds]; arr[i] = { ...arr[i], amount: Number(e.target.value) }; setS({ ...s, withdrawal_thresholds: arr }) }} />
                  ...
                  {ALL_DAYS.map(d => {
                    ...
                    return (
                      <button key={d} onClick={() => {
                        ...
                        const arr = [...s.withdrawal_thresholds]; arr[i] = { ...arr[i], days }; setS({ ...s, withdrawal_thresholds: arr })
          ...
          <button onClick={() => setS({ ...s, withdrawal_thresholds: [...(s.withdrawal_thresholds ?? []), { amount: 0, days: ['monday','tuesday','wednesday','thursday','friday'] }] })}
            style={{ width: '100%', padding: '0.75rem', borderRadius: 'var(--r-sm)', border: '1.5px dashed var(--sand-3)', background: 'none', fontSize: '0.875rem', fontWeight: 600, color: 'var(--emerald)', cursor: 'pointer' }}>
            + Add threshold
          </button>
        </div>

        {/* Claim fees per channel */}
        <div className="card" style={{ padding: '1.25rem' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1rem' }}>
            <div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '0 1rem' }}>
            {(['daily','fixed','referral','checkin','salary','admin_gift'] as const).map(type => (
              <Input key={type} label={`${type.charAt(0).toUpperCase() + type.slice(1)} (%)`} type="number"
                value={String(s.p2p_claim_fees?.[type] ?? 0)}
