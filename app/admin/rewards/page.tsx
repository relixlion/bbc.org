...
export default function AdminGiftRewards() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/admin/users${q}`).then(r => r.json()).then((d: UserRow[]) => {
      if (Array.isArray(d)) {
        setUsers(d)
        // Auto-select if user_id passed from community page
        if (!preloaded) {
  ...
  async function gift() {
    ...
    const res = await fetch('/api/admin/rewards', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ user_id: selected.id, amount: Number(amount), label: label || 'Admin gift' }) })
    const data = await res.json()
    if (!res.ok) setError(data.error)
    else { setSuccess(`₦${Number(amount).toLocaleString()} gifted to ${maskPhone(selected.phone)}`); setAmount(''); setLabel(''); setSelected(null) }
    setSending(false)
  }
        ...
        {selected && (
          <div className="card" style={{ padding: '1.25rem' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr auto', gap: '0.5rem' }}>
              ...
              <button onClick={() => setSelected(null)} style={{ padding: '0 1rem', borderRadius: 'var(--r-md)', border: '1.5px solid var(--sand-3)', background: 'none', fontSize: '0.875rem', fontWeight: 600, color: 'var(--ink-3)', cursor: 'pointer' }}>Cancel</button>
            </div>
          </div>
        )}
      </div>
